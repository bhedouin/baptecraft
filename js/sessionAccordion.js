function loadSessionAccordion(json, error) {
    ...
    if (!sessions.length) {
        sessionTable.innerHTML = `<tr><td>Aucune donnée</td><td>-</td><td>-</td><td>-</td></tr>`;
        return;
    }
...
function onOpenSession(i, sessions) {
    ...
    return function () {
        ...
        setTimeout(function () {
            const session = sessions[i];
            const worldSeries = {name: `Temps de Jeu par Monde`, colorByPoint: true, data: session.world_series};
...
function createAccordionTitle(i, session) {
    ...
    return `<tr id="session_h_${i}" aria-controls="session_t_${i}" aria-expanded="false" 
                class="clickable collapsed ${style}" data-bs-target="#session_t_${i}" data-bs-toggle="collapse">
                <td>${session.name}${session.first_session ? ` <i title="Inscription (Première session)" class="far fa-calendar-plus"></i>` : ``}</td>
                ...
                <td colspan="4">
                    <div class="collapse row" data-bs-parent="#tableAccordion" id="session_t_${i}">
                        <div class="col-xs-12  col-sm-12 col-md-6 col-lg-6">
                            <p><i class="col-teal far fa-fw fa-clock"></i> Session Terminée<span class="float-end"><b>${session.end}</b></span></p>
                            <p><i class="col-green far fa-fw fa-clock"></i> Longueur<span class="float-end"><b>${session.length}</b></span></p>
                            <p><i class="col-grey far fa-fw fa-clock"></i> Temps AFK<span class="float-end"><b>${session.afk_time}</b></span></p>
                            <p><i class="col-green fa fa-fw fa-server"></i> Serveur<span class="float-end"><b>${session.server_name}</b></span></p>
                            ${session.avg_ping ? `<p><i class="col-amber fa fa-fw fa-signal"></i> Latence moyenne<span class="float-end"><b>` + session.avg_ping + `</b></span></p>` : ``}
                            <br>
                            <p><i class="col-red fa fa-fw fa-crosshairs"></i> Kills de Joueurs<span class="float-end"><b>${session.player_kills.length}</b></span></p>
                            <p><i class="col-green fa fa-fw fa-crosshairs"></i> Kills de Mobs<span class="float-end"><b>${session.mob_kills}</b></span></p>
                            <p><i class=" fa fa-fw fa-skull"></i> Morts<span class="float-end"><b>${session.deaths}</b></span></p>
                            <hr>
...
function createKillsTable(player_kills) {
    ...
    if (!player_kills.length) {
        table += `<tr><td>Vide</td><td>-</td><td>-</td><td>-</td></tr>`
    }
