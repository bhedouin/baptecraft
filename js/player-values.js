...
function loadPlayerOverviewValues(json, error) {
    ...
    $(element).find("#data_titles").replaceWith((data.operator ? `<i class="col-blue fab fa-fw fa-superpowers"></i> Opérateur` : '') + (data.banned ? `<i class="col-red fa fa-fw fa-gavel"></i> Banni(e)` : ''));
...
function createConnectionsTableBody(connections) {
    ...
    if (connections.length === 0) {
        table += `<tr><td>Aucune donnée</td><td>-</td></tr>`
    }
...
// Lowercase due to locale translation: Serveur
function loadserverAccordion(json, error) {
    ...
    if (!servers.length) {
        serverTable.append(`<tr><td>Aucune donnée</td><td>-</td><td>-</td><td>-</td></tr>`)
    }
...
function onOpenserver(i, servers) {
    ...
    return function () {
        ...
        setTimeout(function () {
            var server = servers[i];
            var worldSeries = {name: `Temps de Jeu par Monde`, colorByPoint: true, data: server.world_pie_series};
...
function createserverAccordionBody(i, server) {
    ...
    return `<tr class="collapse" data-bs-parent="#tableSAccordion" id="server_t_` + i + `">` +
        ...
        (server.operator ? `<p><i class="col-blue fab fa-fw fa-superpowers"></i> Opérateur</p>` : ``) +
        (server.banned ? `<p><i class="col-red fas fa-fw fa-gavel"></i> Banni(e)</p>` : ``) +
        ...
        `<p><i class="col-green far fa-fw fa-clock"></i> Temps de Jeu<span class="float-end"><b>` + server.playtime + `</b></span></p>` +
        `<p><i class="col-grey far fa-fw fa-clock"></i> Temps AFK<span class="float-end"><b>` + server.afk_time + `</b></span></p>` +
        `<p><i class="col-teal far fa-fw fa-clock"></i> Session la plus Longue<span class="float-end"><b>` + server.longest_session_length + `</b></span></p>` +
        `<p><i class="col-teal far fa-fw fa-clock"></i> Session Médiane<span class="float-end"><b>` + server.session_median + `</b></span></p>` +
        `<br>` +
        `<p><i class="col-red fa fa-fw fa-crosshairs"></i> Kills de Joueurs<span class="float-end"><b>` + server.player_kills + `</b></span></p>` +
        `<p><i class="col-green fa fa-fw fa-crosshairs"></i> Kills de Mobs<span class="float-end"><b>` + server.mob_kills + `</b></span></p>` +
        `<p><i class=" fa fa-fw fa-skull"></i> Morts<span class="float-end"><b>` + server.deaths + `</b></span></p>` +
